...
def set_figsize(figsize=(3.5, 2.5)):
    ...
    use_svg_display()
    dltools.plt.rcParams['figure.figsize'] = figsize
...
def plot(X, Y=None, xlabel=None, ylabel=None, legend=None, xlim=None,
    ...
    set_figsize(figsize)
    axes = axes if axes else dltools.plt.gca()
...
def synthetic_data(w, b, num_examples):
    ...
    X = dltools.normal(0, 1, (num_examples, len(w)))
    y = dltools.matmul(X, w) + b
    y += dltools.normal(0, 0.01, y.shape)
    return X, dltools.reshape(y, (-1, 1))


# Defined in file: ./chapter_linear-networks/linear-regression-scratch.md
def linreg(X, w, b):
    """The linear regression model."""
    return dltools.matmul(X, w) + b


# Defined in file: ./chapter_linear-networks/linear-regression-scratch.md
def squared_loss(y_hat, y):
    """Squared loss."""
    return (y_hat - dltools.reshape(y, y_hat.shape)) ** 2 / 2
...
def show_images(imgs, num_rows, num_cols, titles=None, scale=1.5):
    ...
    _, axes = dltools.plt.subplots(num_rows, num_cols, figsize=figsize)
    axes = axes.flatten()
...
def accuracy(y_hat, y):
    ...
    if len(y_hat.shape) > 1 and y_hat.shape[1] > 1:
        y_hat = dltools.argmax(y_hat, axis=1)
    cmp = dltools.astype(y_hat, y.dtype) == y
    return float(dltools.reduce_sum(dltools.astype(cmp, y.dtype)))
...
def evaluate_accuracy(net, data_iter):
    ...
    for X, y in data_iter:
        metric.add(accuracy(net(X), y), dltools.size(y))
...
def predict_ch3(net, test_iter, n=6):
    ...
    for X, y in test_iter:
        break
    trues = dltools.get_fashion_mnist_labels(y)
    preds = dltools.get_fashion_mnist_labels(dltools.argmax(net(X), axis=1))
    titles = [true + '\n' + pred for true, pred in zip(trues, preds)]
    dltools.show_images(dltools.reshape(X[0:n], (n, 28, 28)), 1, n,
                        titles=titles[0:n])
...
def evaluate_loss(net, data_iter, loss):
    ...
    metric = dltools.Accumulator(2)  # Sum of losses, no. of examples
    for X, y in data_iter:
        out = net(X)
        y = dltools.reshape(y, out.shape)
        l = loss(out, y)
        metric.add(dltools.reduce_sum(l), dltools.size(l))
    return metric[0] / metric[1]


# Defined in file: ./chapter_multilayer-perceptrons/kaggle-house-price.md
DATA_HUB = dict()
DATA_URL = 'http://dltools-data.s3-accelerate.amazonaws.com/'
...
def download_extract(name, folder=None):
    ...
    fname = download(name)
    # fname = "E:/data/aclImdb_v1.tar.gz"
...
def corr2d(X, K):
    ...
    h, w = K.shape
    Y = dltools.zeros((X.shape[0] - h + 1, X.shape[1] - w + 1))
    for i in range(Y.shape[0]):
        for j in range(Y.shape[1]):
            Y[i, j] = dltools.reduce_sum((X[i:i + h, j:j + w] * K))
    return Y
...
def evaluate_accuracy_gpu(net, data_iter, device=None):
    ...
    for X, y in data_iter:
        if isinstance(X, list):
            ...
            X = X.to(device)
        y = y.to(device)
        metric.add(dltools.accuracy(net(X), y), dltools.size(y))
    return metric[0] / metric[1]
...
def train_ch6(net, train_iter, test_iter, num_epochs, lr, device):
    ...
    animator = dltools.Animator(xlabel='epoch', xlim=[1, num_epochs],
                                legend=['train loss', 'train acc', 'test acc'])
    ...
    for epoch in range(num_epochs):
        ...
        for i, (X, y) in enumerate(train_iter):
            ...
            optimizer.step()
            with torch.no_grad():
                metric.add(l * X.shape[0], dltools.accuracy(y_hat, y), X.shape[0])
            timer.stop()
...
dltools.DATA_HUB['time_machine'] = (dltools.DATA_URL + 'timemachine.txt',
...
def predict_ch8(prefix, num_preds, net, vocab, device):
    ...
    get_input = lambda: dltools.reshape(dltools.tensor([outputs[-1]], device=device),
                                        (1, 1))
...
def train_epoch_ch8(net, train_iter, loss, updater, device, use_random_iter):
    ...
    metric = dltools.Accumulator(2)  # Sum of training loss, no. of tokens
    for X, Y in train_iter:
        ...
        metric.add(l * dltools.size(y), dltools.size(y))
...
dltools.DATA_HUB['fra-eng'] = (dltools.DATA_URL + 'fra-eng.zip',
                               '94646ad1522d915e7b0f9296181140edcf86a4f5')


def read_data_nmt():
    """Load the English-French dataset."""
    data_dir = dltools.download_extract('fra-eng')
...
def build_array_nmt(lines, vocab, num_steps):
    ...
    array = dltools.tensor([
        truncate_pad(l, num_steps, vocab['<pad>']) for l in lines])
    valid_len = dltools.reduce_sum(dltools.astype(array != vocab['<pad>'], dltools.int32),
                                   1)
...
class Seq2SeqEncoder(dltools.Encoder):
...
def train_seq2seq(net, data_iter, lr, num_epochs, tgt_vocab, device):
    ...
    net.train()
    animator = dltools.Animator(xlabel='epoch', ylabel='loss',
                                xlim=[10, num_epochs])
    for epoch in range(num_epochs):
        timer = dltools.Timer()
        metric = dltools.Accumulator(2)  # Sum of training loss, no. of tokens
        for batch in data_iter:
            ...
            dec_input = dltools.concat([bos, Y[:, :-1]], 1)  # Teacher forcing
...
def show_heatmaps(matrices, xlabel, ylabel, titles=None, figsize=(2.5, 2.5),
    ...
    fig, axes = dltools.plt.subplots(num_rows, num_cols, figsize=figsize,
                                     sharex=True, sharey=True, squeeze=False)
    for i, (row_axes, row_matrices) in enumerate(zip(axes, matrices)):
        for j, (ax, matrix) in enumerate(zip(row_axes, row_matrices)):
            pcm = ax.imshow(dltools.numpy(matrix), cmap=cmap)
            if i == num_rows - 1:
...
def masked_softmax(X, valid_lens):
    ...
    if valid_lens is None:
        ...
        X = dltools.sequence_mask(X.reshape(-1, shape[-1]), valid_lens,
                                  value=-1e6)
...
class PositionalEncoding(nn.Module):
    def __init__(self, num_hiddens, dropout, max_len=1000):
        ...
        self.P = dltools.zeros((1, max_len, num_hiddens))
        X = dltools.arange(max_len, dtype=torch.float32).reshape(
            -1, 1) / torch.pow(
            10000,
...
class EncoderBlock(nn.Module):
    def __init__(self, key_size, query_size, value_size, num_hiddens,
        ...
        self.attention = dltools.MultiHeadAttention(key_size, query_size,
...
def annotate(text, xy, xytext):
    dltools.plt.gca().annotate(text, xy=xy, xytext=xytext,
...
def show_trace_2d(f, results):
    ...
    x1, x2 = dltools.meshgrid(dltools.arange(-5.5, 1.0, 0.1),
                              dltools.arange(-3.0, 1.0, 0.1))
...
dltools.DATA_HUB['airfoil'] = (dltools.DATA_URL + 'airfoil_self_noise.dat',
                               '76e5be1548fd8222e5074cf0faae75edff8cf93f')


def get_data_ch11(batch_size=10, n=1500):
    data = np.genfromtxt(dltools.download('airfoil'), dtype=np.float32,
                         delimiter='\t')
    data = torch.from_numpy((data - data.mean(axis=0)) / data.std(axis=0))
    data_iter = dltools.load_array((data[:n, :-1], data[:n, -1]), batch_size,
                                   is_train=True)
        ...
        for X, y in data_iter:
            ...
            if n % 200 == 0:
                ...
                animator.add(n / X.shape[0] / len(data_iter),
                             (dltools.evaluate_loss(net, data_iter, loss),))
                timer.start()
    ...
    for _ in range(num_epochs):
        for X, y in data_iter:
            ...
            if n % 200 == 0:
                timer.stop()
                animator.add(n / X.shape[0] / len(data_iter),
                             (dltools.evaluate_loss(net, data_iter, loss) / 2,))
                timer.start()
    print(f'loss: {animator.Y[0][-1]:.3f}, {timer.avg():.3f} sec/epoch')
    ...
    def resnet_block(in_channels, out_channels, num_residuals,
        ...
        for i in range(num_residuals):
            if i == 0 and not first_block:
                blk.append(
                    dltools.Residual(in_channels, out_channels, use_1x1conv=True,
                                     strides=2))
            else:
...
def train_batch_ch13(net, X, y, loss, trainer, devices):
    ...
    train_acc_sum = dltools.accuracy(pred, y)
...
dltools.DATA_HUB['hotdog'] = (dltools.DATA_URL + 'hotdog.zip',
...
def box_corner_to_center(boxes):
    ...
    w = x2 - x1
    h = y2 - y1
    boxes = dltools.stack((cx, cy, w, h), axis=-1)
    return boxes
...
def box_center_to_corner(boxes):
    ...
    x2 = cx + 0.5 * w
    y2 = cy + 0.5 * h
    boxes = dltools.stack((x1, y1, x2, y2), axis=-1)
    return boxes
...
def bbox_to_rect(bbox, color):
    ...
    return dltools.plt.Rectangle(xy=(bbox[0], bbox[1]), width=bbox[2] - bbox[0],
...
def multibox_prior(data, sizes, ratios):
    ...
    size_tensor = dltools.tensor(sizes, device=device)
    ratio_tensor = dltools.tensor(ratios, device=device)
...
def offset_boxes(anchors, assigned_bb, eps=1e-6):
    ...
    c_anc = dltools.box_corner_to_center(anchors)
    c_assigned_bb = dltools.box_corner_to_center(assigned_bb)
    offset_xy = 10 * (c_assigned_bb[:, :2] - c_anc[:, :2]) / c_anc[:, 2:]
    offset_wh = 5 * dltools.log(eps + c_assigned_bb[:, 2:] / c_anc[:, 2:])
    offset = dltools.concat([offset_xy, offset_wh], axis=1)
    return offset
...
dltools.DATA_HUB['banana-detection'] = (
    dltools.DATA_URL + 'banana-detection.zip',
...
def read_data_bananas(is_train=True):
    ...
    data_dir = dltools.download_extract('banana-detection')
...
dltools.DATA_HUB['voc2012'] = (dltools.DATA_URL + 'VOCtrainval_11-May-2012.tar',
...
def load_data_voc(batch_size, crop_size):
    ...
    voc_dir = dltools.download_extract('voc2012',
...
dltools.DATA_HUB['cifar10_tiny'] = (dltools.DATA_URL + 'kaggle_cifar10_tiny.zip',
...
dltools.DATA_HUB['dog_tiny'] = (dltools.DATA_URL + 'kaggle_dog_tiny.zip',
                                '0cb91d09b814ecdc07b50f31f8dcad3e81d6a86d')

# Defined in file: ./chapter_natural-language-processing-pretraining/word-embedding-dataset.md
dltools.DATA_HUB['ptb'] = (dltools.DATA_URL + 'ptb.zip',
                           '319d85e578af0cdc590547f26231e4e31cdf1e42')


def read_ptb():
    data_dir = dltools.download_extract('ptb')
...
def get_negatives(all_contexts, corpus, K):
    counter = dltools.count_corpus(corpus)
...
def batchify(data):
    ...
    return (dltools.reshape(dltools.tensor(centers),
                            (-1, 1)), dltools.tensor(contexts_negatives),
            dltools.tensor(masks), dltools.tensor(labels))
...
dltools.DATA_HUB['glove.6b.50d'] = (dltools.DATA_URL + 'glove.6B.50d.zip',
                                    '0b8703943ccdb6eb788e6f091b8946e82231bc4d')

dltools.DATA_HUB['glove.6b.100d'] = (dltools.DATA_URL + 'glove.6B.100d.zip',
                                     'cd43bfb07e44e6f27cbcc7bc9ae3d80284fdaf5a')

dltools.DATA_HUB['glove.42b.300d'] = (dltools.DATA_URL + 'glove.42B.300d.zip',
                                      'b5116e234e9eb9076672cfeabf5469f3eec904fa')

dltools.DATA_HUB['wiki.en'] = (dltools.DATA_URL + 'wiki.en.zip',
...
class TokenEmbedding:
    ...
    def _load_embedding(self, embedding_name):
        ...
        data_dir = dltools.download_extract(embedding_name)
        ...
        return idx_to_token, dltools.tensor(idx_to_vec)

    def __getitem__(self, tokens):
        indices = [
            self.token_to_idx.get(token, self.unknown_idx)
            for token in tokens]
        vecs = self.idx_to_vec[dltools.tensor(indices)]
        return vecs
...
class BERTEncoder(nn.Module):
    def __init__(self, vocab_size, num_hiddens, norm_shape, ffn_num_input,
        ...
        for i in range(num_layers):
            self.blks.add_module(
                f"{i}",
                dltools.EncoderBlock(key_size, query_size, value_size,
...
class _WikiTextDataset(torch.utils.data.Dataset):
    def __init__(self, paragraphs, max_len):
        ...
        paragraphs = [
            dltools.tokenize(paragraph, token='word') for paragraph in paragraphs]
        sentences = [
            sentence for paragraph in paragraphs for sentence in paragraph]
        self.vocab = dltools.Vocab(
...
def load_data_wiki(batch_size, max_len):
    num_workers = dltools.get_dataloader_workers()
    # data_dir = dltools.download_extract('wikitext-2', 'wikitext-2')
...
def load_data_imdb(batch_size, num_steps=500):
    ...
    train_features = torch.tensor([
        dltools.truncate_pad(vocab[line], num_steps, vocab['<pad>'])
        for line in train_tokens])
    test_features = torch.tensor([
        dltools.truncate_pad(vocab[line], num_steps, vocab['<pad>'])
        for line in test_tokens])
    train_iter = dltools.load_array((train_features, torch.tensor(train_data[1])),
                                    batch_size)
    test_iter = dltools.load_array((test_features, torch.tensor(test_data[1])),
...
def predict_sentiment(net, vocab, sentence):
    sentence = torch.tensor(vocab[sentence.split()], device=dltools.try_gpu())
...
dltools.DATA_HUB['SNLI'] = ('https://nlp.stanford.edu/projects/snli/snli_1.0.zip',
...
class SNLIDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset, num_steps, vocab=None):
        ...
        if vocab is None:
            self.vocab = dltools.Vocab(all_premise_tokens + all_hypothesis_tokens,
    ...
    def _pad(self, lines):
        return torch.tensor([
            dltools.truncate_pad(self.vocab[line], self.num_steps,
...
dltools.DATA_HUB['pokemon'] = (dltools.DATA_URL + 'pokemon.zip',
